...
1. **Crossbeam MCP (required).** The verified tools on the current surface: `find_partner_recommendations` (partners + `ei_signals` for a deal/account), `find_partner_shared_contacts` (partner-shared contacts with roles and an `in_own_crm` flag), `find_overlapping_partners` (partner-level metrics), `find_overlapping_accounts_and_leads` (account lists, each row now carrying its own CRM `record_id`, including the reciprocal give), `get_partner_overlaps_shared_context` (a partner's shared custom fields on one account, e.g. account tier or renewal date), `get_account_context` (your own CRM fields, returned as `own_fields`, for the account itself), `search_crossbeam_knowledge`. Match on these suffixes; the prefix varies per installation and tool sets differ between installs, so confirm what is present on the first call. Account resolution is built into the `find_*` tools via fuzzy `account_name` / `account_domain` matching — they return `ClarificationRequired` with candidate records — so no separate account-lookup tool is needed. If no Crossbeam MCP is connected, stop and tell the user to connect and authenticate. Do not proceed until confirmed.
